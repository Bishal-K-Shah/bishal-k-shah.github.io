...
interface HomeContentProps {
  initialPosts: Post[];
  categoriesList: Category[]; // Receive only the list of category names (strings)
}

function HomeContent({ initialPosts, categoriesList }: HomeContentProps) {
  ...
  useEffect(() => {
    ...
    if (categoryParam) {
      ...
      const isValid = categoriesList.includes(categoryParam as Category);
      if (isValid) {
...
export default function Home({
  ...
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center">Loading...</div>}>
      <HomeContent initialPosts={initialPosts} categoriesList={categoriesList} />
    </Suspense>
  );
}
